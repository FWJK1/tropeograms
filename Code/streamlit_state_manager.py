...
def initialize_state(matrix): 
    tropers = {
        title: GenreTroperator(matrix=matrix, movie_path=path, title=title) for title, path in movies.items()
    }
...
## define some helper funcs and pool into single calc ***
...
def get_dynamic_range(troperators, taus):
    ranges = [movie.get_dynamic_y_range(taus) for movie in troperators]
    return max(ranges)
